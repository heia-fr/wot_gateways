...
import org.ws4d.coap.client.BasicClientChannelManager;
...
public class BasicCoapSocketHandler implements SocketHandler {
	...
	public BasicCoapSocketHandler(ChannelManager channelManager, int port)
			throws IOException {
		this.channelManager = channelManager;
		dgramChannel = DatagramChannel.open();
		dgramChannel.setOption(StandardSocketOptions.SO_REUSEADDR, true);
		dgramChannel.socket().bind(new InetSocketAddress(port)); // port can be
																	// 0, then a
																// free port
																	// is chosen
		this.localPort = dgramChannel.socket().getLocalPort();
		this.localAddr = dgramChannel.socket().getLocalAddress();
	...
	public BasicCoapSocketHandler(ChannelManager channelManager)
			throws IOException {
	...
	protected class WorkerThread extends Thread {
		...
		public void close() {
			...
			try {
				dgramChannel.socket().disconnect();
				dgramChannel.socket().close();
				dgramChannel.disconnect();
				dgramChannel.close();
				dgramChannel=null;
			} catch (IOException e) {
				e.printStackTrace();
			...
			if(channelManager instanceof BasicClientChannelManager){
				//free some place in the memory
				((BasicClientChannelManager)channelManager).remove(getLocalPort());
			}
		...
		public void run() {
			...
			while (dgramChannel != null) {
				...
				dgramBuffer.clear();
				try {
					addr = (InetSocketAddress) dgramChannel
							.receive(dgramBuffer);
				} catch (IOException e1) {
		...
		private void handleIncommingMessage(ByteBuffer buffer,
			...
			new Thread(new HandleIncommingMessageThread(b, addr)).start();
		}
		...
		private void sendUdpMsg(CoapMessage msg) {
			 ...
			 * any Map!
			 */
			logger.info("Prepare message serialization");
			try {
	...
	private void addClientChannel(ClientChannel channel) {
		clientChannels.put(
	...
	public void removeClientChannel(ClientChannel channel) {
		clientChannels.remove(new ChannelKey(channel.getRemoteAddress(),
	...
	public ClientChannel connect(CoapClient client, InetAddress remoteAddress,
		...
		if (clientChannels
				.containsKey(new ChannelKey(remoteAddress, remotePort))) {
			// channel already exists
			logger.warn("Cannot connect: Client channel already exists");
			return null;
		}

		ClientChannel channel = new BasicCoapClientChannel(this, client,
	...
	private class HandleIncommingMessageThread implements Runnable{
	
		
		private final Logger logger = LoggerFactory
				.getLogger(HandleIncommingMessageThread.class);
		private ByteBuffer buffer;
		private InetSocketAddress addr;
		
		
		public HandleIncommingMessageThread(ByteBuffer buffer,
				InetSocketAddress addr) {
			super();
			this.buffer = buffer;
			this.addr = addr;
		...
		public void run() {
			CoapMessage msg;
			try {
				msg = AbstractCoapMessage.parseMessage(this.buffer.array(),
			...
			if (msg.isRequest()) {
				...
				if (packetType == CoapPacketType.ACK
					...
					System.out.println("Invalid Packet Type: Request can not be in a ACK or a RST packet");
					return;
				}
				 ...
				 */
				if (BasicCoapSocketHandler.this.workerThread.isRemoteDuplicate(msgKey)) {
					BasicCoapSocketHandler.this.workerThread.retransmitRemoteDuplicate(msgKey);
					return;
				}
				...
				if (channel == null) {
					...
					channel = ((ServerChannelManager) channelManager)
							.createServerChannel(BasicCoapSocketHandler.this,
									msg, addr.getAddress(), addr.getPort());
					if (channel != null) {
						...
						CoapChannel fakeChannel = new BasicCoapServerChannel(
								BasicCoapSocketHandler.this, null,
						...
						sendMessage(rstMsg);
						logger.info("Server refused CoAP connection");
						System.out.println("Server refused CoAP connection");
						return;
				...
				if (packetType == CoapPacketType.RST) {
					...
					System.out.println("Invalid Packet Type: RST packet must be empty");
					return;
				}
				...
				if (packetType == CoapPacketType.CON) {
					...
					ClientChannel channel = clientChannels.get(new ChannelKey(
							addr.getAddress(), addr.getPort()));
					if (channel == null) {
						logger.warn("Could not find channel of incomming separat response: message dropped");
						System.out.println("Could not find channel of incomming separat response: message dropped");
						return;
					}
					msg.setChannel(channel);
					channel.handleMessage(msg);
				 ...
				 * host
				 */
				if (BasicCoapSocketHandler.this.workerThread.isHostDuplicate(msgId)) {
				 ...
				 * as answer to a CON
				 */
				synchronized (BasicCoapSocketHandler.this.workerThread.timeoutConMsgMap) {
					BasicCoapSocketHandler.this.workerThread.timeoutConMsgMap.remove(msgId);
				}

				ClientChannel channel = clientChannels.get(new ChannelKey(addr
						.getAddress(), addr.getPort()));
				if (channel == null) {
					logger.warn("Could not find channel of incomming response: message dropped");
					System.out.println("Could not find channel of incomming response: message dropped");
					return;
				}
				msg.setChannel(channel);
				channel.handleMessage(msg);
				...
				if (packetType == CoapPacketType.CON
					...
					System.out.println("Invalid Packet Type: CON or NON packets cannot be empty");
					return;
				}
				...
				CoapChannel channel = clientChannels.get(new ChannelKey(addr
						.getAddress(), addr.getPort()));
				if (channel == null) {
				...
				System.out.println("Invalid Message Type: not a request, not a response, not empty");
			}
